...
const Book = ({ isOpen, currentPage, setCurrentPage }) => {
  ...
  return (
    <div className={`book-float-wrapper ${isOpen ? 'open' : 'closed'}`}>
      ...
      {/* Back cover — rendered as a single centered page (like front cover at page 0) */}
      {currentPage === 6 && (
        <div className="page-side cover-back" style={{
          ...
          <div style={{ marginTop: '16px', fontSize: '0.72rem', color: 'rgba(197,168,128,0.7)', letterSpacing: '0.12em', cursor: 'pointer', fontFamily: 'var(--font-serif)' }} onClick={(e) => handlePageTurn('prev', e)}>↩ Reopen</div>
        </div>
      )}
